    ...
    if epoch % N_eval_epoch == 0:
        ...
        with torch.no_grad():
            for i in range(snt_te):
                ...
                pout = Variable(
                    torch.zeros(N_fr + 1, class_lay[-1]).float().cuda().contiguous()
                )
                count_fr = 0
                count_fr_tot = 0
                while end_samp < signal.shape[0]:
                    ...
                    if count_fr == Batch_dev:
                        ...
                        count_fr = 0
                        sig_arr = (
                            torch.zeros([Batch_dev, wlen]).float().cuda().contiguous()
                        )
        ...
        torch.save(checkpoint["CNN_model_par"], os.path.join(output_folder, "CNN_model_par.pth"))
        torch.save(checkpoint["DNN1_model_par"], os.path.join(output_folder, "DNN1_model_par.pth"))
        torch.save(checkpoint["DNN2_model_par"], os.path.join(output_folder, "DNN2_model_par.pth"))
