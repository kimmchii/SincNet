...
model_file = "./sincnet_models/SincNet_TIMIT_original/model_raw.pkl"  # This is the model to use for computing the d-vectors (it should be pre-trained using the speaker-id DNN)
cfg_file = "./sincnet_models/SincNet_TIMIT_original/SincNet_TIMIT.cfg"  # Config file of the speaker-id experiment used to generate the model
    ...
    for i in range(snt_te):
        ...
        while end_samp < signal.shape[0]:
            ...
            if count_fr == Batch_dev:
                ...
                dvects[count_fr_tot - Batch_dev : count_fr_tot, :] = DNN1_net(
                    CNN_net(inp)
                )
                count_fr = 0
                sig_arr = torch.zeros([Batch_dev, wlen]).float().to(device).contiguous()
